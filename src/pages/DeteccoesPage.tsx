...
import { buscarEventosGDACS } from '../services/gdacsService';
...
const FONTE_CONFIG = {
  ...
  GDACS:        { label: 'GDACS', cor: 'bg-orange-500/10 border-orange-500/30 text-orange-400' },
} as const;
...
function DeteccaoCard({
  ...
  const escopoCor = ESCOPO_CONFIG[deteccao.escopo] ?? 'bg-amber-500/10 border-amber-500/30 text-amber-400';
...
type Estado =
  | { status: 'idle' }
  | { status: 'loading' }
  | { status: 'ok'; dados: DeteccaoExterna[] }
  | { status: 'erro'; mensagem: string };

export function DeteccoesPage() {
  const [estado, setEstado] = useState<Estado>({ status: 'idle' });
  const [filtroFonte, setFiltroFonte] = useState<'Todos' | 'NASA-EONET' | 'USGS' | 'GDACS'>('Todos');
  ...
  async function carregar() {
    ...
    try {
      ...
      if (eonet.status === 'fulfilled') dados.push(...eonet.value);
      if (usgs.status === 'fulfilled') dados.push(...usgs.value);
      if (gdacs.status === 'fulfilled') dados.push(...gdacs.value);

      dados.sort(
        (a, b) => new Date(b.dataDeteccao).getTime() - new Date(a.dataDeteccao).getTime(),
      );

      if (dados.length === 0 && eonet.status === 'rejected' && usgs.status === 'rejected' && gdacs.status === 'rejected') {
        setEstado({
          status: 'erro',
  ...
  function handleCriarOcorrencia(d: DeteccaoExterna) {
    const fonteLabel = d.fonte === 'NASA-EONET' ? 'NASA EONET' : d.fonte === 'GDACS' ? 'GDACS' : 'USGS';
    const descricao =
      `${d.tipoEvento} detectado automaticamente via ${fonteLabel}. ` +
      `Satélites detectores: ${d.satelite}. ` +
      `Localização: ${d.latitude.toFixed(4)}, ${d.longitude.toFixed(4)}. ` +
      (d.magnitude !== undefined ? `Magnitude: ${d.magnitude.toFixed(1)}. ` : '') +
      `Detecção em: ${formatarDataHora(d.dataDeteccao)}.`;

    const dataInicio = d.dataDeteccao.split('T')[0];

    navigate('/ocorrencias/nova', {
      state: {
        deteccao: { descricao, dataInicio },
        satelitesSugeridos: d.sateliteIds,
      },
    });
  }
  ...
  const totalGdacs = deteccoes.filter((d) => d.fonte === 'GDACS').length;

  return (
    <div className="space-y-6">
      <PageHeader
        title="Detecções via Satélite"
        description="Eventos naturais na América do Sul detectados via NASA EONET, USGS (sismos) e GDACS (desastres globais)"
        action={
          <Button
            variant="secondary"
            size="sm"
      ...
      {estado.status === 'ok' && totalEonet === 0 && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-300/80">
          <span className="font-semibold text-amber-300">NASA EONET:</span> o catálogo da NASA não
          registra eventos ativos na América do Sul no momento (cobertura focada em eventos de
          grande escala). Os eventos abaixo são do USGS e GDACS.
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          { label: 'USGS Sismos',valor: totalUsgs,         cor: 'text-violet-400',  bg: 'bg-violet-500/10 border-violet-500/30' },
          { label: 'GDACS',      valor: totalGdacs,        cor: 'text-orange-400',  bg: 'bg-orange-500/10 border-orange-500/30' },
      ...
      {estado.status === 'ok' && deteccoes.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {([
            { key: 'Todos',     label: `Todos (${deteccoes.length})` },
            { key: 'NASA-EONET',label: `NASA EONET (${totalEonet})` },
            { key: 'USGS',      label: `USGS Sismos (${totalUsgs})` },
            { key: 'GDACS',     label: `GDACS (${totalGdacs})` },
          ] as const).map(({ key, label }) => (
            <button
              key={key}
              onClick={() => setFiltroFonte(key)}
      ...
      {estado.status === 'ok' && deteccoes.length > 0 && (
        <div className="flex flex-wrap gap-3 text-xs text-slate-500">
          <span>Escopo:</span>
          {(['Brasil', 'América do Sul'] as const).map((e) => (
